...
def pick6():
    ...
    for x in range(
        6
    ):  # loop through a total of 6 times for 6 random numbers between 1 and 99
        ticket_numbers.append(
            random.randint(1, 99)
        )  # add each loops random number to the empty ticket_numbers list
...
def num_matches(
    winning, ticket
):  # function with two parameters of "winning" and "ticket"
    matches = 0  # set the number of matches to 0 to start off with

    for number in range(
        6
    ):  # for each number in the comparison, loop through each number and compare each same indice of both tickets to determine matches
...
winning_ticket = (
    pick6()
)  # create the winning_ticket numbers with the pick6() function that's previously been created
