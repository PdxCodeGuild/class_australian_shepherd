...
function addToDom() {
    ...
    todoObject.forEach((task) => {
        if (task.completed === false) {
            todoSection.innerHTML += `<div><button class="todostyle" onclick="completeTodo(${task.id})">${task.name}</button><p class="todostyle"><button onclick="deleteTodo(${task.id})">Delete</button></p></div>`;
        }
        let completedSection = document.querySelector("#completed-section");
        if (task.completed === true) {
            completedSection.innerHTML += `<div><button class="todostyle" onclick="undoComplete(${task.id})">${task.name}</button><p class="todostyle"><button onclick="deleteTodo(${task.id})">Delete</button></p></div>`;
        }
    });
}
